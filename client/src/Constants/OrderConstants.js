export const ORDER_CREATE_REQUEST = 'ORDER_CREATE_REQUEST';
export const ORDER_CREATE_SUCCESS = 'ORDER_CREATE_SUCCESS';
export const ORDER_CREATE_FAIL = 'ORDER_CREATE_FAIL';
export const ORDER_CREATE_RESET = 'ORDER_CREATE_RESET';

export const ORDER_DETAILS_REQUEST = 'ORDER_DETAILS_REQUEST';
export const ORDER_DETAILS_SUCCESS = 'ORDER_DETAILS_SUCCESS';
export const ORDER_DETAILS_FAIL = 'ORDER_DETAILS_FAIL';

export const ORDER_MINE_LIST_REQUEST = 'ORDER_MINE_LIST_REQUEST';
export const ORDER_MINE_LIST_SUCCESS = 'ORDER_MINE_LIST_SUCCESS';
export const ORDER_MINE_LIST_FAIL = 'ORDER_MINE_LIST_FAIL';

export const ORDER_LIST_REQUEST = 'ORDER_LIST_REQUEST';
export const ORDER_LIST_SUCCESS = 'ORDER_LIST_SUCCESS';
export const ORDER_LIST_FAIL = 'ORDER_LIST_FAIL';

export const ORDER_LIST_ADMIN_SUCCESS = 'ORDER_LIST_ADMIN_SUCCESS';
export const ORDER_LIST_ADMIN_FAIL = 'ORDER_LIST_ADMIN_FAIL';
export const ORDER_LIST_ADMIN_REQUEST = 'ORDER_LIST_ADMIN_REQUEST';

export const ORDER_LIST_BY_STATUS_ADMIN_SUCCESS = 'ORDER_LIST_BY_STATUS_ADMIN_SUCCESS';
export const ORDER_LIST_BY_STATUS_ADMIN_FAIL    = 'ORDER_LIST_BY_STATUS_ADMIN_FAIL';
export const ORDER_LIST_BY_STATUS_ADMIN_REQUEST = 'ORDER_LIST_BY_STATUS_ADMIN_REQUEST';

export const ORDER_DELETE_REQUEST = 'ORDER_DELETE_REQUEST';
export const ORDER_DELETE_SUCCESS = 'ORDER_DELETE_SUCCESS';
export const ORDER_DELETE_FAIL = 'ORDER_DELETE_FAIL';
export const ORDER_DELETE_RESET = 'ORDER_DELETE_RESET';

export const ORDER_DELIVER_REQUEST = 'ORDER_DELIVER_REQUEST';
export const ORDER_DELIVER_SUCCESS = 'ORDER_DELIVER_SUCCESS';
export const ORDER_DELIVER_FAIL = 'ORDER_DELIVER_FAIL';
export const ORDER_DELIVER_RESET = 'ORDER_DELIVER_RESET';

export const ORDER_STATUS_UPDATE_SUCCESS = 'ORDER_STATUS_UPDATE_SUCCESS';
export const ORDER_STATUS_UPDATE_REQUEST = 'ORDER_STATUS_UPDATE_REQUEST';
export const ORDER_STATUS_UPDATE_FAIL = 'ORDER_STATUS_UPDATE_FAIL';
export const ORDER_STATUS_UPDATE_RESET = 'ORDER_STATUS_UPDATE_RESET';

export const ORDER_STATUS_UPDATE_REQUEST_ADMIN = 'ORDER_STATUS_UPDATE_REQUEST_ADMIN';
export const ORDER_STATUS_UPDATE_FAIL_ADMIN    = 'ORDER_STATUS_UPDATE_FAIL_ADMIN';
export const ORDER_STATUS_UPDATE_SUCCESS_ADMIN = 'ORDER_STATUS_UPDATE_SUCCESS_ADMIN';
export const ORDER_STATUS_UPDATE_RESET_ADMIN   = 'ORDER_STATUS_UPDATE_RESET_ADMIN';
